		...
		<v-dialog v-model="openPostResultModal" persistent max-width="900">
			<v-card>
				<v-card-title class="headline px-3">
					{{ programYear(studentInfo.PROGRAMMEID) }}
					<v-spacer></v-spacer>
				...
				<div
					class="grey--text text--darken-2 text-subtitle-2 ml-3 mb-3"
				>
					Name: {{ studentInfo.SNAME }}
				</div>
				<first-year
					v-if="studentInfo.PROGRAMMEID === 1"
					:studentDetails="studentInfo"
					v-on:childToParent="onChildClick"
					@openModalStatus="closePostResultModal"
				/>
				<second-year
					v-if="studentInfo.PROGRAMMEID === 2"
					:studentDetails="studentInfo"
					v-on:childToParent="onChildClick"
					@openModalStatus="closePostResultModal"
				/>
				<third-year
					v-if="studentInfo.PROGRAMMEID === 3"
					:studentDetails="studentInfo"
					v-on:childToParent="onChildClick"
					@openModalStatus="closePostResultModal"
				/>
				<fourth-year
					v-if="studentInfo.PROGRAMMEID === 4"
					:studentDetails="studentInfo"
					v-on:childToParent="onChildClick"
					@openModalStatus="closePostResultModal"
				/>
				<fifth-year
					v-if="studentInfo.PROGRAMMEID === 5"
					:studentDetails="studentInfo"
					v-on:childToParent="onChildClick"
					@openModalStatus="closePostResultModal"
				/>
				<llm-first-year
					v-if="studentInfo.PROGRAMMEID === 6"
					:studentDetails="studentInfo"
					v-on:childToParent="onChildClick"
					@openModalStatus="closePostResultModal"
				/>
				<llm-second-year
					v-if="studentInfo.PROGRAMMEID === 7"
					:studentDetails="studentInfo"
